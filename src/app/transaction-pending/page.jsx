...
export default function TransactionPendingPage() {
  ...
  /**
   * Mengambil data transaksi pending dari API sesuai filter, pencarian, dan pagination.
...
const cards = [
  ...
  const fetchChart = () => {
    ...
    fetch(`/api/v1/transactions/pending/chart?period=${chartPeriod}`)
      .then(res => res.json())
  ...
  return (
    <div className="flex flex-col gap-4 sm:gap-8 px-4 sm:px-0">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0 absolute top-[75px] right-[35px] mt-2">
