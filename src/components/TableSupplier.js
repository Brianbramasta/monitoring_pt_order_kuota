      ...
      <div className="w-full">
        <div className="overflow-x-auto w-full">
          <table className="w-full min-w-[640px] border-collapse" style={{fontFamily: 'Poppins, Arial, sans-serif', fontSize: 14}}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={mode === 'compare' ? 7 : 6} className="text-center py-8 text-gray-400 text-sm">Loading...</td></tr>
              ) : data.length === 0 ? (
                <tr><td colSpan={mode === 'compare' ? 7 : 6} className="text-center py-8 text-gray-400 text-sm">Tidak ada data</td></tr>
              ) : data.map((item, idx) => (
                <tr key={item.supplier_code} className={idx%2===1 ? 'bg-[#F8FAFB]' : ''}>
                  ...
                  <td className="px-2 sm:px-4 py-3 sm:py-2 text-xs sm:text-sm text-[#222] border-[#E0E0E0] whitespace-nowrap text-center">{item.no}</td>
